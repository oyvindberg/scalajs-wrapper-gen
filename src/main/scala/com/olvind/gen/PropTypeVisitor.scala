...
case class PropTypeVisitor(n: CompName, o: ObjectNode, jsContent: String, is: Seq[Import]) extends MyNodeVisitor(o){
  var propTypes: Option[Map[PropName, OriginalProp]] = None
  o accept this

  def mapPropType(start: Int, ps: List[PropertyNode]): List[(PropName, OriginalProp)] = {
    ps match {
      case Nil => Nil
      case p :: pt =>
        ...
        (PropName(p.getKeyName) -> OriginalProp(n, typeS, commentOS)) +: mapPropType(p.getValue.getFinish + 1, pt)
    }
  }
