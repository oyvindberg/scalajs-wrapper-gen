...
object ButtonSharedComponent extends SharedComponent {
  ...
  val inheritEvents = List[OutField](
    ...
    OptField("onTouchTap",             OutParamClass("ReactTouchEventH => Callback"), Some(JsonField("onTouchTap", Some("function(event)"), "optional", "Called when a touch tap event occurs on the button.")))
  )
}
